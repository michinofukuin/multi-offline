...
class new_3(object):
    def __init__(self, observation_spec, action_spec, num_agent, eval_env, config):
        ...
        self._optimizers['behaviour'] = torch.optim.Adam(self.behaviour_network.parameters(), lr=1e-4)
    ...
    def q_loss_2(self, o_with_a_id, s, o_next_with_id, s_next, r, mask, result={}):
        ...
        expected_q_total = r + self._gamma * mask * v_next_target_total.detach()
        q_loss = ((q_total - expected_q_total.detach()) ** 2).mean()
        result.update({'q_loss_reward': q_loss, 'q_total_reward': q_total.mean()})
        return result
    ...
    def v_loss_2(self, q_target_1, v_1, result={}, s_a=None):
        error = q_target_1.detach() - v_1
        if s_a is not None:
            w, b =self._mix_target_network_2(s_a)
        ...
        result.update({'v_loss_reward' : v_loss})
        return result
    
    def v_loss_1(self, z, w_target, v_values, result={}):
        max_z = torch.max(z)
    ...
    def get_contri(self, s, o, o_with_id):
        ...
        q_values = self._q_target_con(o_with_a_id)
        if self.is_mix:
            w, b = self._mix_target_network_2(s)
    ...
    def train_step(self, o, s, a, r, mask, s_next, o_next, a_next):
        ...
        if self.is_mix:
            cc = self.log_alpha.exp().view(1, self._num_agent, 1).detach()
            z = 1 / cc * (w_target.detach() * q_target_1.detach() - w_target.detach() * v_values).to(self._device)
        else:
            z = 1
        ...
        loss_result = self.q_loss_2(o_with_a_id, s, o_next_with_id, s_next, r, mask, result=loss_result)
        self._optimizers['q_con'].zero_grad()
        loss_result['q_loss_reward'].backward()
        nn.utils.clip_grad_norm_(self._q_param_con, self._grad_norm_clip)
        self._optimizers['q_con'].step()

        loss_result = self.v_loss_1(z, w_target, v_values, result=loss_result)
        ...
        loss_result = self.v_loss_2(q_target_2, v_con, result=loss_result)
        self._optimizers['v_con'].zero_grad()
        loss_result['v_loss_reward'].backward()
